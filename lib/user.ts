...
export const getUserData = cache(async ({ userId }: userDataProps) => {
  ...
  for (const acc of accounts) {
    ...
    const cleanTxns = txns.map((t) => ({
      ...t,
      amount:
        t.amount && typeof (t.amount as any).toNumber === "function"
...
export const handleCurrentId = async (searchParamsAccountId?: string) => {
  ...
  const cookieName = `currentAccountId_${userId}`;

  const lastAccountId = cookieStore.get(cookieName)?.value;

  // Case 1: User explicitly picked a new account
  if (searchParamsAccountId && searchParamsAccountId.trim() !== "") {
    if (searchParamsAccountId !== lastAccountId) {
      // Only update cookie if different from last for this user
      cookieStore.set(cookieName, searchParamsAccountId, {
        httpOnly: true,
        sameSite: "lax",
      });
    }
  ...
  // Case 2: No search param → fallback to last cookie for this user
