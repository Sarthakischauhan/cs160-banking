...
export default async function AccountManagementPage({ searchParams }: { searchParams: { [key: string]: string | undefined } }) {
    ...
    const accountData = await prisma.account.findMany({
        where: {
            ...
            ...(search ? { OR: [{ account_id: search }, { customer_id: search }] } : {}),
                ...
                <div className="p-10">
                    ...
                    <form method="GET" className="flex flex-col gap-4">
                        ...
                        <div className="grid grid-cols-3 w-full gap-4">
                            <TextFilter label={"Search by ID"} name="search" value={search} placeholder="Enter ID" />
                            <RangeFilter label={"Balance"} minName="minBalance" maxName="maxBalance" minValue={minBalance} maxValue={maxBalance} minPlaceholder="Minimum Balance" maxPlaceholder="Maximum Balance" prefix="$" />
                            <RangeFilter label={"Date"} minName="minDate" maxName="maxDate" minValue={minDate} maxValue={maxDate} type="date" />
                        </div>
                        <div className="grid grid-cols-3 w-full gap-4">
                            <div>
                                <SelectFilter label="Account Type" name="accountType" options={Object.keys(AccountType)} value={accountType} />
                            </div>
                        </div>
                    </form>
