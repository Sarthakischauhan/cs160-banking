...
export function ReportCard() {
  ...
  const drawBarChart = (
    ...
    monthlySpending.forEach((monthData, index) => {
      const barHeight =
        maxSpending > 0 ? (monthData.total / maxSpending) * chartHeight : 0;
    ...
    if (averageSpending > 0 && maxSpending > 0) {
      const avgY =
        chartY + chartHeight - (averageSpending / maxSpending) * chartHeight;
      ...
      doc.text(
        avgText,
        chartX + chartWidth - doc.getTextWidth(avgText) - 2,
        avgY - 2,
      );
      doc.setTextColor(0, 0, 0);
    }
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const averageSpending =
        monthlySpending.length > 0 ? totalSpending / monthlySpending.length : 0;
      ...
      if (monthlySpending.length > 0) {
        ...
        summaryItems.forEach((item) => {
          doc.text("• " + item, headerLeft + 4, currentY, {
        ...
        const highestMonth = monthlySpending.reduce((a, b) =>
          a.total > b.total ? a : b,
        );
        const lowestMonth = monthlySpending.reduce((a, b) =>
          a.total < b.total ? a : b,
        );
        ...
        insights.forEach((insight) => {
          doc.text("• " + insight, headerLeft + 4, currentY, {
            maxWidth: pageWidth - 42,
          });
          currentY += 6;
    ...
    <>
      <Card className="h-full">
        <CardHeader>
          ...
          <CardDescription>
            View trends and a summary of your past spendings
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div>
            <span>
              Generate a comprehensive financial report to analyze your spending
              patterns and trends.
            </span>
          </div>
        </CardContent>
      ...
      <Dialog open={isDialogOpen} onOpenChange={handleDialogClose}>
        <DialogContent className="max-w-5xl h-[90vh] flex flex-col p-0 gap-0">
          <DialogHeader className="px-6 pt-6 pb-4 shrink-0">
            ...
            <DialogDescription>
              Preview your generated financial report below. You can download it
