...
export async function POST(req: Request) {
  try {
    ...
    if (!front_image || !back_image) {
      return new Response("Please upload front and back image", {
        status: 400,
      });
    }
    const validImageTypes = ["image/jpeg", "image/jpg", "image/png"];
    if (
      !validImageTypes.includes(front_image.type) ||
      !validImageTypes.includes(back_image.type)
    ) {
      return new Response(
        "Invalid image type. Only JPEG , JPG & PNG are allowed.",
        { status: 415 },
      );
    }
    ...
    if (!amount || amount <= 0) {
      return new Response("Deposit amount must be a positive number", {
        status: 400,
      });
    }
